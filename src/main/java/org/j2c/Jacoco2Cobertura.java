...
import org.j2c.core.XMLMain;
import org.j2c.utils.Validator;
...
@Command(name = "Jacoco2Cobertura", mixinStandardHelpOptions = true, version = "Jacoco2Cobertura 1.0.0", description = "Convert Jacoco xml file to Cobertura xml.")
public class Jacoco2Cobertura implements Runnable {

    @Option(required = true, names = {"-j", "--jacoco"}, description = "Path to the jacoco xml file.", arity = "1..*")
    private String jacocoFile;

    @Option(names = {"-d", "--destination"}, description = "path to save Cobertura xml file.", defaultValue = ".")
    private String destination;

    @Option(required = false, names = {"-s", "--source"}, description = "Path to java source root. If program have more than one src directory pass all with comma seperated", arity = "1..*", split = ",", defaultValue = "src/main/java")
